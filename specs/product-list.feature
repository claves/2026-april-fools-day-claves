Feature: 商品一覧ページ
  ...
  Scenario: 商品カードがランダムに傾いて表示される
    Then 商品カードはそれぞれ異なる角度で傾いている

  Scenario: 商品の表示順がランダムになる
    When ページをリロードする
    Then 商品の表示順が前回と異なる可能性がある

  # === ソート・表示件数（意図的に機能しない） ===

  Scenario: ソート順を変更しても商品順は変わらない
    When ソート順を「運気順」に変更する
    Then 商品の並び順は変わらない

  Scenario: 表示件数を変更しても表示数は変わらない
  ...
  Scenario: 同じ商品を複数回カートに追加する
    When 同じ商品の「カートに追加」ボタンを2回クリックする
    Then カート内の商品数量が2になる

  # === 今すぐ購入ボタン ===

  Scenario: 今すぐ購入ボタンで購入画面に遷移する
